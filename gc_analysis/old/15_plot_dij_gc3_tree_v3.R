...
make_color_function <- function(gc3_values) {
  pal <- colorRampPalette(c("black", "#04E466"))(100)
...
plot_tree_pdf <- function(tree, edge_keys, pdf_path, title_str, gc3_values, gradient_steps = 40,
  ...
  # Keep the same branch geometry as v2, but allocate more page width to the
  # tree and reduce internal margins so the tree extends closer to the legend.
  pdf(pdf_path, width = 15, height = fig_h)
  layout(matrix(c(1, 2), nrow = 1), widths = c(6.8, 0.75))

  par(mar = c(0.5, 0.25, 2.5, 0.05))
  plot.phylo(
    tree,
    type = "phylogram",
  ...
  # Very small left margin in the legend panel shifts the legend contents left.
  par(mar = c(2.2, 0.0, 2.5, 0.2))
  plot.new()
  ...
  if (length(valid_gc3) > 0) {
    ...
    # Wider color bar, shifted left within the legend panel.
    for (i in seq_len(99)) {
      rect(xleft = 0.02, ybottom = y[i], xright = 0.42, ytop = y[i + 1],
           col = pal[i], border = NA)
    }

    text(0.46, 0.85, labels = sprintf("%.2f", max(valid_gc3)), adj = 0, cex = 0.8)
    text(0.46, 0.15, labels = sprintf("%.2f", min(valid_gc3)), adj = 0, cex = 0.8)
    text(0.02, 0.93, labels = "GC3", adj = 0, cex = 0.9, font = 2)
    text(0.02, 0.08, labels = "low = black, high = #04E466", adj = 0, cex = 0.65)

    if (!is.na(ancestral_gc_display)) {
      text(
        0.02,
        0.02,
        labels = sprintf("Ancestral GC = %.2f", ancestral_gc_display),
        adj = 0,
        cex = 0.70,
        font = 2
      )
