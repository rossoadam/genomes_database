#!/usr/bin/env python3
"""
15_calculate_gc_divergence_v4.py
...
def summarize_gc4_detail_file(path: Path) -> pd.DataFrame:
    """
    Summarize GC4 by species from a per-ortholog/detail TSV.
    ...
    across those rows. If one row per species is present, sd_gc4 is NA because
    a standard deviation cannot be calculated from a single value.
    """
...
class NhPhymlGcDivergence:
    ...
    def export_master(self, gc3_summary: pd.DataFrame, dianc_df: pd.DataFrame) -> None:
        ...
        if self.gc4_detail is not None:
            if not self.gc4_detail.exists():
                raise FileNotFoundError(f"GC4 detail TSV does not exist: {self.gc4_detail}")

            gc4_summary = summarize_gc4_detail_file(self.gc4_detail)
            write_tsv(gc4_summary, self.outdir / "gc4_summary.tsv")

            # Check GC4 row counts against the GC3-derived n_orthologs, but keep
            # only one n_orthologs column in the final output.
            count_check = master[["genus_species", "n_orthologs"]].merge(
                gc4_summary[["genus_species", "n_gc4_values"]],
                on="genus_species",
                how="left",
            )
            mismatches = count_check[
                count_check["n_gc4_values"].notna()
                & (count_check["n_gc4_values"] != count_check["n_orthologs"])
            ]

            if not mismatches.empty:
                preview = mismatches.head(10).to_string(index=False)
                message = (
                    "[WARN] Some species have different GC3 and GC4 ortholog counts. "
                    "The final output keeps only n_orthologs from GC3. First mismatches:\n"
                    f"{preview}"
                )
                if self.strict:
            ...
            master = master.merge(
                gc4_summary[["genus_species", "mean_gc4", "sd_gc4"]],
                on="genus_species",
                how="left",
            )
        else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional per-ortholog/detail TSV from 16_gc4_from_alignment.py. "
            "The script summarizes gc4 by species as mean_gc4 and sd_gc4."
        ),
    )
    parser.add_argument(
        "--gc4-species-summary",
        dest="gc4_detail_legacy",
        type=Path,
        default=None,
        help=argparse.SUPPRESS,
    )
...
def main() -> None:
    ...
    calculator = NhPhymlGcDivergence(
        ...
        strict=args.strict,
        gc4_detail=(args.gc4_detail or args.gc4_detail_legacy).expanduser().resolve() if (args.gc4_detail or args.gc4_detail_legacy) else None,
    )
