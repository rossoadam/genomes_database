...
#
# Optional:
#   --nhphyml-dir /path/to/nhphyml_output
...
nhphyml_dir <- if ("nhphyml_dir" %in% names(args)) args$nhphyml_dir else NA_character_
...
# Layout tuning. These defaults keep the v2 geometry but reduce wasted space
# between the tree and legend.
tree_x_padding <- if ("tree_x_padding" %in% names(args)) as.numeric(args$tree_x_padding) else 1.12
if (is.na(tree_x_padding) || tree_x_padding < 1.02) tree_x_padding <- 1.12

pdf_width <- if ("pdf_width" %in% names(args)) as.numeric(args$pdf_width) else 13
if (is.na(pdf_width) || pdf_width < 8) pdf_width <- 13
...
if (!is.na(nhphyml_dir) && !dir.exists(nhphyml_dir)) {
  stop(sprintf("nhphyml directory not found: %s", nhphyml_dir))
}
...
log_msg("Tree x padding: %.3f", tree_x_padding)
log_msg("PDF width: %.3f", pdf_width)
if (!is.na(node_numbers_file)) log_msg("node_numbers file: %s", node_numbers_file)
if (!is.na(nhphyml_dir)) log_msg("nhphyml dir: %s", nhphyml_dir)
...
parse_one_lk_ancestral_gc <- function(path) {
  lines <- readLines(path, warn = FALSE)
  hit <- grep("Ancestral", lines, value = TRUE)
  if (length(hit) == 0) return(NA_real_)

  nums <- regmatches(hit[1], gregexpr("[-+]?[0-9]*\\.?[0-9]+", hit[1]))[[1]]
  if (length(nums) == 0) return(NA_real_)

  val <- as.numeric(nums[length(nums)])
  if (is.na(val)) return(NA_real_)

  # nhPhyML .lk files usually report proportions, e.g. 0.403571.
  # Convert to percent so it matches gc3_summary.tsv scale.
  if (val <= 1) val <- val * 100
  val
}

read_ancestral_gc_from_nhphyml <- function(nhphyml_dir) {
  if (is.na(nhphyml_dir)) return(NA_real_)

  lk_files <- list.files(
    nhphyml_dir,
    pattern = "\\.phylip_nhPhyml\\.lk$|_nhPhyml\\.lk$|\\.lk$",
    full.names = TRUE
  )

  log_section("Ancestral GC from nhPhyML .lk files")
  log_msg("Found %d .lk files in %s", length(lk_files), nhphyml_dir)

  if (length(lk_files) == 0) return(NA_real_)

  vals <- vapply(lk_files, parse_one_lk_ancestral_gc, numeric(1))
  vals <- vals[!is.na(vals)]

  log_msg("Parsed ancestral GC from %d .lk files", length(vals))
  if (length(vals) == 0) return(NA_real_)

  ancestral_gc <- mean(vals)
  log_msg("Ancestral GC used for figure display: %.6f", ancestral_gc)
  log_msg("Ancestral GC min across .lk files: %.6f", min(vals))
  log_msg("Ancestral GC max across .lk files: %.6f", max(vals))
  log_msg("First 10 .lk files parsed: %s", paste(head(basename(lk_files), 10), collapse = ", "))
...
plot_tree_pdf <- function(tree, edge_keys, pdf_path, gc3_values, gradient_steps = 40,
                          ancestral_gc_display = NA_real_, tree_x_padding = 1.12,
                          pdf_width = 13) { # , title_str
  n_tips <- length(tree$tip.label)
  fig_h <- max(8, n_tips * 0.19)
  fnt_size <- min(1.1, 200 / n_tips)
  valid_gc3 <- gc3_values[!is.na(gc3_values)]
  color_fun <- make_color_function(gc3_values)

  pdf(pdf_path, width = pdf_width, height = fig_h)

  # Give the tree most of the page, but keep the legend panel immediately
  # adjacent. A narrower PDF plus a tighter x.lim reduces the large blank band
  # that appeared between tree tips and the legend.
  layout(matrix(c(1, 2), nrow = 1), widths = c(5.4, 1))

  par(mar = c(1, 0.3, 3, 0.05), xpd = NA)

  tree_depth <- max(node.depth.edgelength(tree), na.rm = TRUE)
  tree_xlim <- c(0, tree_depth * tree_x_padding)
  ...
  draw_vertical_connectors(tree, last_plot, edge_keys, gc3_values, color_fun, lwd = 6)
  ...
  if (length(valid_gc3) > 0) {
    ...
    # Shift legend content left within its panel and make the color bar wider.
    bar_x0 <- 0.37
    bar_x1 <- 0.68
    text_x <- 0.37

    for (i in seq_len(99)) {
      rect(xleft = bar_x0, ybottom = y[i], xright = bar_x1, ytop = y[i + 1],
           col = pal[i], border = NA)
    }

    text(text_x, 0.86, labels = sprintf("%.2f", max(valid_gc3)), adj = 0, cex = 1.5)
    text(text_x, 0.14, labels = sprintf("%.2f", min(valid_gc3)), adj = 0, cex = 1.5)
    text(bar_x0, 0.9, labels = "GC3", adj = 0, cex = 2, font = 2)
    # text(bar_x0, 0.08, labels = "low = black, high = #04E466", adj = 0, cex = 0.65)

    if (!is.na(ancestral_gc_display)) {
      text(
        -5.75,
        0.8,
        labels = sprintf("Ancestral GC3 = %.2f", ancestral_gc_display),
        adj = 0,
        cex = 1.5,
        font = 2
      )
...
ancestral_gc_display <- read_ancestral_gc_from_nhphyml(nhphyml_dir)
if (is.na(ancestral_gc_display) && "ancestral" %in% names(gc3_values)) {
  log_section("Ancestral GC fallback")
  ancestral_gc_display <- as.numeric(gc3_values["ancestral"])
  log_msg("Using ancestral row from gc3_summary.tsv for figure display: %.6f", ancestral_gc_display)
}
if (!is.na(ancestral_gc_display)) {
  gc3_values["ancestral"] <- ancestral_gc_display
}
...
if (!is.na(ancestral_gc_display)) {
  log_msg("Ancestral GC displayed on figure: %.6f", ancestral_gc_display)
} else {
  log_msg("No ancestral GC value available for figure display.")
}
...
plot_tree_pdf(
  ...
  ancestral_gc_display = ancestral_gc_display,
  tree_x_padding = tree_x_padding,
  pdf_width = pdf_width
)
