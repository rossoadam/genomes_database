...
def apply_user_added_mass_overrides(master: pd.DataFrame) -> pd.DataFrame:
    """
    Add/overwrite user-curated mass values that are missing from the natural
    history mass files.

    This does not create species rows by itself; master_output.tsv still
    represents species present in the GC-tree subset unless
    --include-all-manifest-species is used.
    """
    overrides = {
...
class NhPhymlGcDivergence:
    ...
    def export_master(self, gc3_summary: pd.DataFrame, dianc_df: pd.DataFrame) -> None:
        ...
        master = apply_user_added_mass_overrides(master)

        if "Phrynocephalus_guinanensis" not in set(master["genus_species"].astype(str)):
            print(
                "[WARN] Phrynocephalus_guinanensis was not present in master_output.tsv. "
                "The Ji et al. mass override can only be applied when the species is present "
                "in the GC-tree subset or when --include-all-manifest-species includes it."
            )
