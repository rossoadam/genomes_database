...
class NhPhymlGcDivergence:
    ...
    def index_nhphyml_files(self) -> None:
        ...
        complete = sorted(phylip_orthologs & set(self.tree_files) & set(self.lk_files))

        if not complete:
            raise ValueError(
                "No complete ortholog set found. Expected matching files ending in:\n"
                f"  {self.phylip_suffix}\n"
                f"  {self.tree_suffix}\n"
                f"  {self.lk_suffix}"
            )

        missing_tree = sorted(phylip_orthologs - set(self.tree_files))
        missing_lk = sorted(phylip_orthologs - set(self.lk_files))

        if self.strict and (missing_tree or missing_lk):
            raise ValueError(
                f"Missing nhPhyML outputs for some .phylip files. "
                f"missing_tree={len(missing_tree)}, missing_lk={len(missing_lk)}"
            )
