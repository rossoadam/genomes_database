...
class NhPhymlGcDivergence:
    ...
    def export_master(self, gc3_summary: pd.DataFrame, dianc_df: pd.DataFrame) -> None:
        ...
        # Hard-coded literature mass override requested by user.
        # Source note: Ji et al. paper, value provided by user.
        mask_phryno = master["genus_species"] == "Phrynocephalus_guinanensis"
        if mask_phryno.any():
            if "mass_meiri" not in master.columns:
                master["mass_meiri"] = pd.NA
            if "mass_title" not in master.columns:
                master["mass_title"] = pd.NA
            if "mass_preferred" not in master.columns:
                master["mass_preferred"] = pd.NA
            if "mass_source_preferred" not in master.columns:
                master["mass_source_preferred"] = pd.NA

            # Preserve title/meiri if they exist, but fill preferred mass for regression.
            master.loc[mask_phryno, "mass_preferred"] = master.loc[mask_phryno, "mass_preferred"].fillna(11.1)
            master.loc[
                mask_phryno & master["mass_source_preferred"].isna(),
                "mass_source_preferred",
            ] = "Ji_et_al_user_added"
            master.loc[
                mask_phryno & (master["mass_source_preferred"] == ""),
                "mass_source_preferred",
            ] = "Ji_et_al_user_added"
