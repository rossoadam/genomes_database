...
if (length(missing_args) > 0) {
  ...
  cat("    --dij /path/to/dij_results.json_or_tsv \\\n")
...
edge_child_key <- function(tree, child_node_number) {
  if (child_node_number <= length(tree$tip.label)) {
    return(normalize_species_name(tree$tip.label[child_node_number]))
  }
  return(paste0("node_", child_node_number))
}

make_edge_child_keys <- function(tree) {
  ...
  for (i in seq_len(nrow(tree$edge))) {
    ...
    if (child <= n_tips) {
      ...
      if (!is.null(tree$node.label) &&
        ...
        # This fallback uses ape's internal node ID. If the input tree does not
        # have explicit node labels, this may differ from node_ IDs created by
        # the Python nhPhyML parser. The log file will show these mismatches.
...
color_from_gc3 <- function(gc3_values, keys) {
  pal <- colorRampPalette(c("black", "lightblue"))(100)
...
# ── 3. Read tree, dij, and GC3 summary ───────────────────────────────────────
...
edge_keys <- make_edge_child_keys(tree)
...
interesting_patterns <- c("acritoscincus", "carinascincus", "anolis")
