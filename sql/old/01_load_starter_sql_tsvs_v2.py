...
and can optionally truncate the starter/window tables before loading.
...
def table_input_files(tsv_dir: Path, table: str) -> List[Path]:
    """Return input files for one table. Supports legacy table.tsv and v12 table/*.tsv.gz chunks."""
    files: List[Path] = []
    ...
    if table_dir.exists() and table_dir.is_dir():
        files.extend(sorted(table_dir.glob("*.tsv")))
        files.extend(sorted(table_dir.glob("*.tsv.gz")))
