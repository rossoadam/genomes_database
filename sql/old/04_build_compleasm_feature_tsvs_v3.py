...
def load_genomes_lookup(genomes_tsv: Path) -> Dict[str, Dict[str, object]]:
    ...
    for _, row in df.iterrows():
        ...
        lookup[accession] = item
        lookup[root_acc(accession)] = item
    return lookup
...
def resolve_metadata_rows(metadata_path: Path, manifest_accessions: List[str]) -> Tuple[pd.DataFrame, List[str]]:
    ...
    metadata_df = metadata_df.rename(columns={accession_col: "accession", cds_col: "cds_fasta", full_col: "full_table"}).copy()
    allowed = set(str(a).strip() for a in manifest_accessions)
    allowed_roots = {root_acc(a) for a in allowed}
    ...
    exact = metadata_df[metadata_df["accession"].isin(allowed)].copy()
    fallback = metadata_df[(~metadata_df["accession"].isin(allowed)) & (metadata_df["accession_root"].isin(allowed_roots - set(exact["accession_root"])))].copy()
    if not fallback.empty:
        fallback = fallback.sort_values(["accession_root", "accession_version"], ascending=[True, False]).drop_duplicates("accession_root")
    resolved = pd.concat([exact, fallback], ignore_index=True)
    if resolved.empty:
        return resolved, sorted(allowed_roots)
    resolved = resolved.sort_values(["accession_root", "accession_version"], ascending=[True, False]).drop_duplicates("accession_root").reset_index(drop=True)
    return resolved, sorted(allowed_roots - set(resolved["accession_root"]))
...
def attach_genome_fna(metadata_rows: pd.DataFrame, genomes_metadata_path: Path, genomes_dir: Path) -> pd.DataFrame:
    ...
    gm["accession_root"] = gm["genome_metadata_accession"].astype(str).str.strip().map(root_acc)
    keep_cols = ["accession_root", "path_to_fna"]
    extra_cols = [c for c in ["organism_name", "species", "species_ncbi"] if c in gm.columns]
    gm = gm[keep_cols + extra_cols].drop_duplicates("accession_root")
    out = metadata_rows.merge(gm, on="accession_root", how="left", suffixes=("", "_genome_metadata"))
...
def main() -> None:
    ...
    for _, meta in metadata_rows.iterrows():
        ...
        lookup = genomes_lookup.get(accession) or genomes_lookup.get(root_acc(accession))
        if lookup is None:
        ...
        try:
            validity = load_validity_rows(find_validity_tsv(cds_fasta))
        ...
        for odb12_id in all_ortholog_ids:
            ...
            if not (passes_qc and seq and record and record.status == "Single" and record.exons and sequence_pk is not None):
                continue

            valid_summary_row = {"ortholog_pk": this_pk, "gc": gc, "gc3": gc3, "gc4": gc4, "length": ortholog_length}
