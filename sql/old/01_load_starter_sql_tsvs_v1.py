#!/usr/bin/env python3
"""
Load TSV outputs from 00_build_starter_sql_tsvs_v10.py into a MySQL database.

This is intentionally standalone for troubleshooting. It validates TSV relationships
before loading, loads tables in dependency order, and can optionally truncate the
starter/window tables before loading.
...
def validate_headers(tsv_dir: Path, tables: Sequence[str]) -> None:
    ...
    for table in tables:
        ...
        if header != expected:
            errors.append(
                f"Header mismatch for {table}.tsv\n"
                f"  expected: {expected}\n"
                f"  observed: {header}"
            )
    if errors:
...
def load_table(cursor, table: str, path: Path) -> int:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tsv-dir", required=True, help="Directory containing TSVs from 00_build_starter_sql_tsvs_v10.py")
...
def main() -> None:
    ...
    for table in tables:
        print(f"  {table}.tsv\t{row_counts.get(table, 0)} rows")
    ...
    try:
        with conn.cursor() as cursor:
            ...
            for table in tables:
                path = tsv_dir / f"{table}.tsv"
                loaded = load_table(cursor, table, path)
                print(f"  loaded {table}.tsv into {table}: {loaded} row(s)")
