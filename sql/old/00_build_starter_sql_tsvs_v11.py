...
This script creates load-ready TSVs:
  ...
  6. analysis_run.tsv
  7. window_set.tsv
  8. genomic_windows.tsv
  9. gc_window_stats.tsv
  10. sequence_summary.tsv
  11. genome_summary.tsv
...
def build_analysis_run_rows(run_pk: int, analysis_name: str, mask_mode: str, min_callable_frac: float, notes: str) -> List[Dict[str, object]]:
    return [{
        ...
        "software": "00_build_starter_sql_tsvs_v10.py",
        "software_version": "v10",
        "mask_mode": mask_mode,
        "gap_break_bp": r"\N",
...
def build_window_tables(
    ...
    """Build window_set, genomic_windows, gc_window_stats, sequence_summary, and genome_summary rows.
    ...
    for genome_pk in sorted(genome_pk_to_accession):
        accession = genome_pk_to_accession[genome_pk]
        fasta_path = find_fasta(accession, fasta_path_lookup)
        print(f"[INFO] Building windows for {accession}: {fasta_path}", file=sys.stderr)
        genome = load_genome(fasta_path)
        ...
        for size_bp, step_bp in zip(window_sizes, step_sizes):
            ...
            window_set_rows.append({
                ...
                "seq_scope": seq_scope,
                "notes": f"Non-overlapping/sliding windows for sequence_type in {sorted(allowed_types)}; created for GC variance decay analyses.",
            })
            window_set_pk += 1
            ...
            # sequence_summary has median_callable_fraction, but genome_summary schema does not.
    ...
    return window_set_rows, genomic_window_rows, gc_window_rows, sequence_summary_rows, genome_summary_rows
...
def main() -> None:
    ...
    window_set_rows: List[Dict[str, object]] = []
    genomic_window_rows: List[Dict[str, object]] = []
    gc_window_stats_rows: List[Dict[str, object]] = []
    sequence_summary_rows: List[Dict[str, object]] = []
    genome_summary_rows: List[Dict[str, object]] = []
    ...
    if not args.skip_windows:
        written.update({
            ...
            "genomic_windows.tsv": write_tsv(output_dir / "genomic_windows.tsv", GENOMIC_WINDOWS_COLUMNS, genomic_window_rows),
            "gc_window_stats.tsv": write_tsv(output_dir / "gc_window_stats.tsv", GC_WINDOW_STATS_COLUMNS, gc_window_stats_rows),
            "sequence_summary.tsv": write_tsv(output_dir / "sequence_summary.tsv", SEQUENCE_SUMMARY_COLUMNS, sequence_summary_rows),
            "genome_summary.tsv": write_tsv(output_dir / "genome_summary.tsv", GENOME_SUMMARY_COLUMNS, genome_summary_rows),
        })
