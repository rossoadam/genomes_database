...
run_one() {
  ...
  # create a local temp directory (outside of Google Drive so there are no problems)
  LOCAL_TMP="$(mktemp -d /tmp/compleasm_XXXXXX)"
  trap 'rm -rf "$LOCAL_TMP"' RETURN # this should trigger if compleasm fails. this will clean up the tmp

  TMP_OUT="${LOCAL_TMP}"/out
  mkdir -p "$TMP_OUT"

  # Run compleasm
  compleasm run --assembly_path "$genome" --output_dir "$TMP_OUT" --threads "$THREADS" --lineage "$LINEAGE_ID" --library "$LIBDIR"

  # move the final results back to the google drive out_dir & clean tmp
  mkdir -p "$outdir"
  cp -a "$TMP_OUT"/. "$outdir"/
