...
# ── 6. Root the tree ──────────────────────────────────────────────────────────
#
# The UNREST tree is already rooted by IQ-TREE but on the branch subtending
# geckos + skinks + Xantusia + Hemicordylus. We want the root on the branch
# leading into the gecko clade only, with skinks/Xantusia/Hemicordylus in
# the ingroup. We do this by rerooting using the MRCA of the gecko clade
# directly, rather than anchoring on a single ingroup tip.

if (length(matched_outgroup) == 1) {
  
  cat("Single outgroup taxon - rooting on its branch\n")
  outgroup_node <- which(tree$tip.label == matched_outgroup)
  rooted_tree   <- reroot(tree, outgroup_node)
  
} else {
  
  cat("Multiple outgroup taxa - rerooting on gecko MRCA branch\n")
  
  # Find the MRCA of just the gecko clade
  gecko_mrca <- findMRCA(tree, tips = matched_outgroup)
  cat(sprintf("Gecko MRCA node: %d\n", gecko_mrca))
  
  # reroot() on an internal node roots on the branch subtending that node,
  # placing the root between the gecko clade and everything else
  rooted_tree <- reroot(tree, gecko_mrca)
...
if (!is.rooted(rooted_tree)) {
  stop("Tree is still unrooted after rerooting attempt - check phytools version")
}
