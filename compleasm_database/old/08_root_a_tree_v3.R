...
# ── 6. Root the tree ──────────────────────────────────────────────────────────
#
# Strategy: find the MRCA of all ingroup taxa (everything that is NOT a gecko)
# and reroot on that node. This places the root on the branch separating the
# gecko clade from all other squamates, making geckos the outgroup.
...
if (length(matched_outgroup) == 1) {
  ...
  cat("Multiple outgroup taxa - rerooting on ingroup MRCA\n")
  
  ingroup_mrca <- findMRCA(tree, tips = ingroup_tips)
  
  if (is.null(ingroup_mrca)) {
    stop("Could not find MRCA for ingroup taxa. Check that non-gecko tips are correct.")
  }
  
  cat(sprintf("Ingroup MRCA node: %d\n", ingroup_mrca))
  
  rooted_tree <- reroot(tree, ingroup_mrca)
}
