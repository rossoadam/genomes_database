...
if (length(matched_outgroup) == 1) {
  ...
  cat("Multiple outgroup taxa - rerooting to place geckos as outgroup\n")
  
  ingroup_tip  <- tree$tip.label[!tree$tip.label %in% matched_outgroup][1]
  ingroup_node <- which(tree$tip.label == ingroup_tip)
  
  cat(sprintf("Using ingroup tip as reroot anchor: %s\n", ingroup_tip))
  
  rooted_tree <- reroot(tree, ingroup_node)
